...
static void ui_event_qr_ScreenNet(lv_event_t * e)
{
    ESP_LOGI(TAG, "QR Screen Loaded");
    lv_obj_t * qr = lv_event_get_user_data(e);
    opa_on_Animation(qr, 300);
}
...
esp_err_t app_wifi_start(void)
{
    ...
    if (!provisioned) {
        while (1) {
            ...
            if(!(xEventGroupGetBits(wifi_event_group) & WIFI_PROV_EVENT_START)) {
                wait_connect = true;
                app_wifi_prov_start();
            }
            ...
            lv_obj_t *qr = lv_qrcode_create(ui_ScreenNet, 108, lv_color_black(), lv_color_white());
            lv_qrcode_update(qr, prov_msg, prov_msg_len);
            lv_obj_align(qr, LV_ALIGN_CENTER, 0, 0);
            lv_obj_add_event_cb(ui_ScreenNet, ui_event_qr_ScreenNet, LV_EVENT_SCREEN_LOADED, (void*)qr);
    ...
    if (0 == (WIFI_STA_CONNECT_OK & xEventGroupGetBits(wifi_event_group))) {
        ...
        lv_obj_set_style_img_recolor(ui_comp_get_child(ui_button_down, UI_COMP_BUTTONDOWN_BUTTON_DOWN_ICON),
                                    lv_color_hex(0xDE2C2C), LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_img_recolor_opa(ui_comp_get_child(ui_button_down, UI_COMP_BUTTONDOWN_BUTTON_DOWN_ICON), 255,
...
bool app_wifi_wait_connect(void)
{
    return wait_connect;
}
