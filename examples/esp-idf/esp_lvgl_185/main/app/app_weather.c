...
#define QWEATHER_REQUEST_KEY            "b6b080ffe6204a9a9c12c7571120f4d5"
#define _KEY_VALUE_                     QWEATHER_REQUEST_KEY
...
void weatherupdateTask(void *arg)
{
    ...
    while(1) {
        if(app_wifi_is_connected()) {
            if(app_weather_get_current_info(&weather_info, LOCATION_NUM_SHANGHAI) == ESP_OK) {
                ESP_LOGD(TAG, "Weather temp: %d", weather_info.temp);
                sprintf(weather_temp, "%d", weather_info.temp);
                lv_label_set_text_fmt(ui_comp_get_child(ui_weather_group_1, UI_COMP_WEATHERGROUP1_DEGREE_1), weather_temp);
            } else {
...
void weathergetTask(void *arg)
{
    ESP_LOGI(TAG, "Weather get task started.");
...
{
    CHECK(QWEATHER_REQUEST_KEY_LEN == strlen(QWEATHER_REQUEST_KEY),
    ...
    xTaskCreatePinnedToCore(weatherupdateTask, "weatherupdateTask", 5 * 1024, NULL, 4, NULL, 0);
    xTaskCreatePinnedToCore(weathergetTask, "weathergetTask", 4 * 1024, NULL, 4, NULL, 1);
    return ESP_OK;
}
